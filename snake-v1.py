...
def computeNextFrame(numFrame,coordonnee, objet, objet_2,objet_3):
    ...
    for p in range(len(objet_2)):
        can.create_image(objet_2[0][0], objet_2[0][1], anchor = NW, image = pomme2)   
    
    for p in range(len(objet_3)):
        can.create_image(objet_3[0][0], objet_3[0][1], anchor = NW, image = pomme3)
...
if __name__ == "__main__":
    ...
    # Second objet (la pomme 2)
    objet_2=[]
    a = randint(1,24)
    b = randint(1,24)
    objet_2.append([a*20, b*20, 0])
    
    # Second objet (la pomme empoisonnée)
    objet_3=[]
    c = randint(1,24)
    d = randint(1,24)
